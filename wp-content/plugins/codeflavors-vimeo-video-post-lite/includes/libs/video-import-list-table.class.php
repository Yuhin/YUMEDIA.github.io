...
class CVM_Video_Import_List_Table extends WP_List_Table{
    ...
    function prepare_items() {
		...
		$args = array(
			'source' 	=> isset($_GET['cvm_source']) ? $_GET['cvm_source'] : false,
			'feed'		=> isset($_GET['cvm_feed']) ? $_GET['cvm_feed'] : false,
			'query'		=> isset($_GET['cvm_query']) ? $_GET['cvm_query'] : false,
			'order'		=> isset($_GET['cvm_order']) ? $_GET['cvm_order'] : false,
    		'results'	=> $per_page,
			'page' 		=> $current_page,
			'search_results' => isset( $_GET['cvm_search_results'] ) ? $_GET['cvm_search_results'] : false,
		);		
