...
function TotalSoftGallery_Image_URL_Clicked()
{
	var nIntervId = setInterval(function(){
		...
		if(code.indexOf('img')>0)
		{
			...
			var src=s[1].split('"');
			if(jQuery('#TotalSoftGallery_Video_Video_1').val() != ''){ jQuery('#TotalSoftGallery_VideoIm_URL_2').val(src[0]); if(jQuery('#TotalSoftGallery_VideoIm_URL_2').val().length>0){ jQuery('#TotalSoftGallery_Image_URL_1').val(''); clearInterval(nIntervId); } } else { jQuery('#TotalSoftGallery_Video_URL_2').val("Video is required!"); jQuery('#TotalSoftGallery_Image_URL_1').val(''); }
		}
	},100)
}
...
function Total_Soft_Gallery_Video_Save_Vid()
{
	...
	else if(TotalSoftGallery_Video_URL_2 == '' && TotalSoftGallery_VideoIm_URL_2 != '')
	{
		alert('You can add only images in gallery in Pro version.');
	}
	else
	{
	...
	jQuery.ajax({
		...
		success: function(response){
			...
			if(data[0]['TotalSoftGalleryV_SetType']=='Grid Video Gallery')
			{
				...
				if(data[0]['TotalSoft_GV_2_26']==''){ data[0]['TotalSoft_GV_2_26']='fadeIn'; }
